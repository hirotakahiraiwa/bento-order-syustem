...
router.post('/login', async (req, res) => {
  ...
  const user = await queryOne(
    'SELECT * FROM employees WHERE employee_number = $1 AND is_active = 1',
    [employee_number]
  );
...
router.get('/me', authenticateToken, async (req, res) => {
  const user = await queryOne(
    'SELECT id, employee_number, name, role, is_bento_target, default_order FROM employees WHERE id = $1',
    [req.user.id]
  );
...
router.put('/password', authenticateToken, async (req, res) => {
  ...
  const user = await queryOne('SELECT * FROM employees WHERE id = $1', [req.user.id]);
  if (!bcrypt.compareSync(current_password, user.password_hash)) {
    return res.status(401).json({ error: '現在のパスワードが正しくありません' });
  }

  const hash = bcrypt.hashSync(new_password, 10);
  await runSql('UPDATE employees SET password_hash = $1, updated_at = NOW() WHERE id = $2',
    [hash, req.user.id]);
